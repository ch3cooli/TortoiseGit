...
UINT CProgressDlg::RunCmdList(CWnd *pWnd, std::vector<CString> &cmdlist, STRING_VECTOR &dirlist, bool bShowCommand, CString *pfilename, bool *bAbort, CGitByteArray *pdata, CGit *git)
{
	UINT ret=0;
	...
	for (size_t i = 0; i < cmdlist.size(); ++i)
	{
		...
		if (bShowCommand)
		{
			...
			if (gitList.empty())
				...
				str = CUnicodeUtils::GetMulti(gitList[i]->m_CurrentDir + _T("\r\n") + cmdlist[i].Trim() + _T("\r\n\r\n"), CP_UTF8);
		...
		if (gitList.empty())
			...
			gitList[i]->RunAsync(cmdlist[i].Trim(),&pi, &hRead, NULL, pfilename);
