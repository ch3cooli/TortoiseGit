...
struct CBugTraqWrapper::Internal
{
...
CBugTraqWrapper::CBugTraqWrapper()
	: m(new Internal())
{
}
...
bool CBugTraqWrapper::CreateProvider(const CBugTraqAssociation& assoc)
{
	m->ClearError();
...
bool CBugTraqWrapper::HasIBugTraqProvider()
{
...
bool CBugTraqWrapper::HasIBugTraqProvider2()
{
	return m->m_BugTraqProvider2 != nullptr;
}

CString CBugTraqWrapper::GetLastErr()
{
	return m->m_LastError;
}

bool CBugTraqWrapper::ValidateParameters(HWND hParentWnd, bool& valid)
{
	m->ClearError();
...
bool CBugTraqWrapper::GetLinkText(HWND hParentWnd, CString& linkText)
{
	m->ClearError();
	if (!HasIBugTraqProvider())
		return m->NoBugTraqProvider();
...
bool CBugTraqWrapper::GetCommitMessage(HWND hParentWnd, CString commonRoot, const STRING_VECTOR& pathList, CString originalMessage, CString& newMessage)
{
	m->ClearError();
	if (!HasIBugTraqProvider())
		return m->NoBugTraqProvider();
...
bool CBugTraqWrapper::GetCommitMessage2(HWND hParentWnd, CString commonURL, CString commonRoot, const STRING_VECTOR& pathList, CString originalMessage, CString bugID,
...
bool CBugTraqWrapper::CheckCommit(HWND hParentWnd, CString commonURL, CString commonRoot, const STRING_VECTOR& pathList, CString commitMessage, CString& errorMessage)
{
	m->ClearError();
	if (!HasIBugTraqProvider2())
		return m->NoBugTraqProvider2();
...
bool CBugTraqWrapper::OnCommitFinished(HWND hParentWnd, CString commonRoot, const STRING_VECTOR& pathList, CString logMessage, ULONG revision, CString& error)
{
	m->ClearError();
	if (!HasIBugTraqProvider2())
		return m->NoBugTraqProvider2();
...
bool CBugTraqWrapper::HasOptions(bool& ret)
{
	m->ClearError();
	if (!HasIBugTraqProvider2())
		return m->NoBugTraqProvider2();
...
bool CBugTraqWrapper::ShowOptionsDialog(HWND hParentWnd, CString& newparameters)
{
	m->ClearError();
	if (!HasIBugTraqProvider2())
		return m->NoBugTraqProvider2();
