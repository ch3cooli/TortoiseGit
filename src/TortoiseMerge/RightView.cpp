...
void CRightView::AddContextItems(CIconMenu& popup, DiffStates state)
{
	...
	if (IsBottomViewGood())
	{
		...
		if (bShow)
		{
			...
			popup.AppendMenuIcon(POPUPCOMMAND_USEYOURANDTHEIRINLINE, IDS_VIEWCONTEXTMENU_USEYOURANDTHEIRINLINE);
			popup.AppendMenuIcon(POPUPCOMMAND_USETHEIRANDYOURINLINE, IDS_VIEWCONTEXTMENU_USETHEIRANDYOURINLINE);
		}
	}
	else
	{
